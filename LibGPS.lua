...
local DUMMY_PIN_TYPE = libName .. "DummyPin"
local LIB_IDENTIFIER_FINALIZE = libName .. "_Finalize"
...
SLASH_COMMANDS["/libgpsdebug"] = function(value)
    ...
    df("[%s] debug mode %s", libName, lib.debugMode and "enabled" or "disabled")
end

local function LogMessage(type, message, ...)
    if not lib.debugMode then return end
    df("[%s] %s: %s", libName, type, zo_strjoin(" ", message, ...))
end
...
local function Initialize() -- wait until we have defined all functions
    ...
    EVENT_MANAGER:UnregisterForEvent(libName .. "_Init", EVENT_PLAYER_ACTIVATED)
    --- Unregister handler from older libGPS, as it is now managed by LibMapPing ( >= 6)
    EVENT_MANAGER:UnregisterForEvent(libName .. "_UnmuteMapPing", EVENT_MAP_PING)
...
local function onLoad(eventCode, addonName)
    ...
    EVENT_MANAGER:UnregisterForEvent(libName, eventCode)
